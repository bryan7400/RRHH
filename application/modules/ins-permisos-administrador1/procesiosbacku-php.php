...
if (is_post()) {
    ...
    $buscar_familiares = $db->query("SELECT * FROM ins_estudiante_familiar
    WHERE estudiante_id='$id_estudiante'
     ")->fetch();

    $array_final = array();
    foreach ($buscar_familiares as $key => $buscar_familiare) {

        $familiar_ids = $buscar_familiare['familiar_id'];
        
        $buscar_familiares = $db->query("SELECT * FROM ins_familiar
            WHERE id_familiar='$familiar_ids'
            ")->fetch();
         ...
         foreach ($buscar_familiares as $key => $buscar_familiare) {
                $familiar_nombre = $buscar_familiare['persona_id'];

                $buscar_personas = $db->query("SELECT * FROM sys_persona
            WHERE id_persona='$familiar_nombre'
            ")->fetch_first();  
               array_push($array_final, $buscar_personas);     
...
$consulta="
SELECT d.*, hd.*, z.*, m.*,hd.`complemento`,apam.*,nombre_curso AS nombre_materias, id_curso AS id_materias,
  ...
  FROM ins_horario_profesor_materia z,
    ...
    ext_curso_inscripcion eci
    LEFT JOIN ext_curso_asignacion eca ON eca.id_curso_asignacion = eci.curso_asignacion_id
...
$informacion_estudiante = $db->query("SELECT *, nombre_curso AS nombre_materia, id_curso AS id_materia FROM ext_curso_inscripcion eci
...
AND estudiante_id = '663' ")->fetch();
   ...
   if($accion == "recuperar_datos"){
        $id_externo = $_POST['id_contrato'];
        $cliente = $db->query("SELECT * FROM rrhh_contrato WHERE id_contrato='$id_externo'")->fetch_first(); 
