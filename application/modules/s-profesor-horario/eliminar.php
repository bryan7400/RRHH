...
if (is_post()) {
	...
	//if (isset($_POST[get_csrf()])) {
		...
		$id_horario_profesor_materia = (isset($_POST['id_a_eliminar'])) ? $_POST['id_a_eliminar'] : 0;
		// Obtiene el modo
		$modo = $db->from('ins_horario_profesor_materia')->where('id_horario_profesor_materia', $id_horario_profesor_materia)->fetch_first();
		...
		if ($modo) {
			...
			$est=$db->query("UPDATE ins_horario_profesor_materia SET estado = 'I', usuario_modificacion = '".$_user['id_user']."', fecha_modificacion = '".$fecha_actual."' WHERE id_horario_profesor_materia = '".$id_horario_profesor_materia."'")->execute();
            ...
            */
			if($est){
               // echo 'ok'.$id_aula_paralelo;
                ...
                echo 'error al eliminar'.$id_horario_profesor_materia;
			...
			if ($db->affected_rows) {
				...
				$db->insert('sys_procesos', array(
					...
					'proceso' => 'd',
					'nivel' => 'm',
					'detalle' => 'Se eliminó el modo la asignacion con identificador número ' . $id_horario_profesor_materia . '.',
					'direccion' => $_location,
