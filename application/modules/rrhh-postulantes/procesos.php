...
if ($boton == "listar_nivel") {
    ...
    $localidad = $db->query("SELECT * FROM sys_paises  where estado='1'    order by nombre_nivel asc")->fetch();
    //$localidad = $db->select('z.*')->from('vista_estudiante_localidad z')->where('id_estudiante', $id_estudiante)->order_by('z.id_estudiante_localidad', 'asc')->fetch();
    echo json_encode($localidad);// order by nombre_aula asc
}
...
if($boton == "asignar_postulacion"){
       /* if (!isset($_POST['dias'])){
            echo 11;exit();
        }*/
        if (isset($_POST['id_postulacion']) ) {//poner mas
            // Obtiene los datos
