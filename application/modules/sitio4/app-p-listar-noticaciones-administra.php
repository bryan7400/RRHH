<?php
//app-p-listar-noticaciones-administra.php
//http://localhost/educhecka/?/sitio/app-p-listar-noticaciones-administra
//usuario:tut
//contrasenia:admin
//id_estudiante:365
// genero:m  
 ...
 */
 ...
 /*
SELECT mat.nombre_materia,concat(per.nombres,per.primer_apellido,per.segundo_apellido), com.* 
...
com.grupo = '0'  
AND 
apam.aula_paralelo_id=3
AND com.estado='A' 
ORDER BY com.fecha_inicio ASC
	...
	if (isset($_POST['usuario']) && isset($_POST['contrasenia'])) {
		...
		if ($usuario) {
            ...
            foreach ($comunicados as $key => $value) {
                ...
                if($persona_ids!='' && $persona_ids!=0 && $persona_ids!=null){
                    $arr_persona=explode(',',$persona_ids);
                //var_dump($persona_ids.' PERSONAS');

                  foreach ($arr_persona as $key => $row) { 

                     if($row==$estudiante_id){
                       //var_dump($estudiante_id.' ____________________________________');
                       $siagregar=true;
                     }
                   }
               }
               ...
               $estados=$value['estados'];
                ...
                if($usuarios!='' && $usuarios!=0 && $usuarios!=null){
                    $arr_usuario=explode(',',$usuarios);
                    $arr_estados=explode(',',$estados);

                //var_dump($usuarios.' USUARIOS');
                //var_dump($estados.' USUARIOS');
                    foreach ($arr_usuario as $key => $rows) { 
                            if($rows==5 && $arr_estados[$key]=='SI'){
                            //var_dump($rows.' ____________________________________');
                               // var_dump('::::::'.$row); 
                                 $siagregar=true;
                             }
                   } 
