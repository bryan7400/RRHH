<?php
  ...
  /*  $materia = $db->select('z.*')->from('pro_materia z')
    //->where('g.id_gestion', $id_gestion)
    ->where('z.estado', 'A')
    ->order_by('z.id_materia', 'asc')->fetch(); */  

$materia = $db->query("SELECT * FROM `pro_materia` ma
INNER JOIN `pro_campo` ca ON ca.`id_campo`=ma.`campo_id` 
WHERE ma.`estado`='A'")->fetch();
