...
$id_profesor=isset($_params[0])?$_params[0]:0; 
...
$sql='';
   if($id_profesor){
        $sql.=" AND asi.`id_asignacion`=$id_profesor";
    }

  //_________nombre de docente________
    //$con="SELECT * FROM pro_profesor pro,sys_persona per WHERE pro.persona_id=per.id_persona";
    $con="SELECT * FROM `per_asignaciones` asi,sys_persona per 
WHERE asi.persona_id=per.id_persona ".$sql;
...
$cod_profesor='';
//$id_profesor='';
foreach($inscritoaa as $nro => $elementod) {
 ...
 //::::: :::::::::: inicio de 1 HORARIO :::::::: ::::::::::: ::::::::::
    ...
    //$id_profesor=$elementod['id_profesor'];
   
}
//var_dump($ape_materno);exit();
...
//if($cct==$tam || $cc==6){//caso de multitabla
    

     //var_dump($tam.' '.$cc.' '.$cct);  exit();
    //$pdf->writeHTML($tablaGeneral, true, false, false, false, '');//caso de multitabla
