...
<div class="row">
    <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
        <div class="page-header">
            <h2 class="pageheader-title">Crear Asignación Curso/Paralelo </h2>
                                ...
                                <div class="input-group-append be-addon">
                                    ...
                                    <div class="dropdown-menu">
                                       ...
                                       <!-- <a href="?/s-curso-paralelo/principal" class="dropdown-item">Listar Curso/Paralelo</a>
                                        ...
                                        <a href="?/s-curso-paralelo/imprimir" class="dropdown-item">Exportar pdf</a>
                                        <!--<a href="?/s-curso-paralelo/imprimirexcel" class="dropdown-item">Exportar Excel</a>-->
  ...
  <div class="modal-dialog">
    ...
    <form action="" class="agregar_paralelo">
      ...
      <div class="modal-body">
            ...
            <input  required type="number" class="form-control" value="" id="Iparacidad" placeholder="Ingrese capacidad de aula"/> <label for="descripcion" class="control-label">Descripcion de aula: </label>
            <input  required type="text" class="form-control" value="" id="descripcion" placeholder="Ej: SAL-1"/>
	...
	<div class="modal-dialog" role="document">
		<div class="modal-content">
            ...
            <input id="id_aula" name="id_asignacion" type="hidden" class="form-control">						
                   ...
                   <div class="control-group">
                   <label class="control-label">Descripcion de aula: </label>
            		    <input id="descrip_aula" name="descrip_aula" type="text" class="form-control" placeholder="Ej: AU12">
					</div>
...
//LLENADOP DE SELECTS CON AULA Y SIS ID
	...
	var nivel = $("#nivel option:selected").val();//this
        ...
        if(aula>0)
            ...
            boton='listar_paralelos_todos_T';
		...
		$.ajax({
            ...
            //alert('ejemplo');
                ...
                $("#Tabla_paralelos").append('<tr><th>N</th><th><th>Turno</th><th>Aula</th>                           <th>Paralelo</th><th>Descripcion Aula</th> <th>Capacidad<th> </tr>');
				//$("#Tabla_paralelos").append('<option value="' + 0 + '">Seleccionar</option>');
                //var cc=1;
				for (var i = 0; i < resp.length; i++) {
                    var contenido = resp[i]['id_aula_paralelo'] + "*" + resp[i]['capacidad']+ "*" + resp[i]['descripcion_aula_paralelo'];//;+ "*" + row['descripcion']+ "*" + row['nombre_nivel'];
  
					$("#Tabla_paralelos").append('<tr><td>'+(i+1)+'</td>          <td><td>' + resp[i]["turno"] + '</td><td>' + resp[i]["nombre_aula"] +' '+resp[i]["nombre_nivel"]+'</td>      <td>' + resp[i]["descripcion"] + '</td> <td>' + resp[i]["descripcion_aula_paralelo"] + '</td> <td><input disabled type="text" class="form-control" value="' + resp[i]["capacidad"] + '"/><td> <a href="#" class="btn btn-warning btn-xs" style="color:white" onclick="abrir_editar('+"'"+contenido+"'"+')"><span class="icon-note"></span></a><a href='+"'#' class='btn btn-danger btn-xs' onclick='abrir_eliminar("+resp[i]["id_aula_paralelo"] +")'><span class='icon-trash'></span></a>"+'</tr>');//
                    //cc++;
				}
...
/*    listar_paralelos_todos();
	function listar_paralelos_todos() {
     aula = $("#aula option:selected").val();
		$.ajax({
			url: '?/s-curso-paralelo/procesos',
			type: 'POST',
			data: {
				'boton': 'listar_paralelos_todos',
				'aula': aula
			},
			dataType: 'JSON',
			success: function(resp){
		    console.log('Listar paralelos '+ resp);
              $("#paralelo").html("");
				$("#paralelo").append('<option value="">Seleccionar</option>');
				for (var i = 0; i < resp.length; i++) {
					$("#paralelo").append('<option value="' + resp[i]["id_paralelo"] + '">' + resp[i]["nombre_paralelo"] + '</option>');
				}
