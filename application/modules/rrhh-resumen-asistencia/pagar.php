...
// Obtiene el empleado
$empleados = $db->select('e.*, p.procedencia, c.cargo')->from('sys_empleados e')->join('gen_procedencias p', 'e.procedencia_id = p.id_procedencia', 'left')->join('per_cargos c', 'e.cargo_id = c.id_cargo', 'left')->where('e.id_empleado', $id_empleado)->fetch();
...
$mes_correspondiente = get_mes_literal($fecha_interior_inicial);
...
<?php $monto_faltas = number_format(($salario/22)*$total_faltas, 2, '.', "");?>
<h3>
	<a href="" id="print" onclick="javascript:printLayer('div-id-name')">Imprimir reporte</a>
</h3>
...
@media print {
	...
	<div class="row">
	<h1 class="text-center"><?= strtoupper($empleado['sucursal']);?></h1>
	...
	<table width="100%">
		<tr>
			...
			<td class="text-right">
				N° DEL EMPLEADOR:  <?= $empleado['codigo'];?>
			</td>
		</tr>
	</table>
	<div class="row">
		<div class="col-md-12">
			<h2 class="text-center">PAPELETA DE PAGO</h2>
			<h4 class="text-center">CORRESPONDIENTE AL MES DE <?= strtoupper($mes_correspondiente); ?> </h4>
			<!-- <hr> -->
		</div>
	</div>
	<div class="row">
		<div class="col-md-8 col-md-offset-2">
			<table class="table">
				<tr>
					...
					<td width="60%"><?= escape($empleado['nombres'] . ' ' . $empleado['paterno'] . ' ' . $empleado['materno']); ?></td>
				</tr>
				<tr>
					<td width="40%">OCUPACIÓN:</td>
					<td width="60%"><?= escape($empleado['cargo']);?></td>
				</tr>
				<tr>
					<td width="40%">CÉDULA DE IDENTIDAD:</td>
					<td width="60%"><?= escape($empleado['ci']);?></td>
				</tr>
				<tr>
					<td width="40%">DÍAS TRABAJADOS:</td>
					<td width="60%"><?= $total_asistencia;?></td>
				</tr>
				<tr>
					<td width="40%">DÍAS NO TRABAJADOS:</td>
					<td width="60%"><?= $total_faltas; ?><td>
				</tr>
				<tr>
					<td width="40%">FECHA DE PAGO:</td>
	...
	<div class="row">
		<div class="col-md-12">
			<table class="table" border="1px">
				<thead>
					<tr>
						<th class="text-center" style="border:1px solid">INGRESO</th>
						<th class="text-center" style="border:1px solid">DESCUENTOS</th>
					</tr>
				</thead>
				<tbody>
					<tr>
						...
						<td>
							<table width="100%">
								...
								<tr>
									<td>Adelantos</td>
									<td class="text-right"><?= $adelanto==''? 0:$adelanto; ?></td>
								</tr>
								<tr>
									<td>Atrasos</td>
									<td class="text-right"><?= convertirHoraMinutos(convert_time($atrasos)); ?></td>
								</tr>
                                <tr>
                                    <td>Faltas</td>
                                    <td class="text-right"><?= $monto_faltas; ?></td>
                                </tr>
								<tfoot>
								    <tr>
								    	<td><strong>TOTAL DESCUENTOS</strong></td>
								    	<td class="text-right"><?= ($adelanto+convertirHoraMinutos(convert_time($atrasos))+$monto_faltas); ?></td>
								    </tr>
								</tfoot>
							</table>
						</td>
			...
			<div class="col-md-4" style="border-style: solid;">
				<div class="row">
					...
					<div class="col-md-6 text-right">
						<?= $salario-($adelanto+convertirHoraMinutos(convert_time($atrasos))+$monto_faltas)?>
					</div>
				</div>
			</div>
		</div>
...
<script>
	function printLayer(layer){
		...
		generator.document.write(layetext.innerHTML.replace("Imprimir"));
