...
if($_user['rol_id']==4){
}else if($_user['rol_id']==5){
}else{
    $persona = $db->query("SELECT*
    FROM sys_persona sp 
    INNER JOIN per_postulacion pp ON sp.postulante_id = pp.id_postulacion
    INNER JOIN per_asignaciones pa ON sp.id_persona = pa.persona_id
    INNER JOIN per_cargos pc ON pa.cargo_id = pc.id_cargo
    WHERE sp.id_persona = $persona_id")->fetch_first();

    $conocimiento = $db->query("SELECT*
    FROM sys_persona sp 
    INNER JOIN per_postulacion pp ON sp.postulante_id = pp.id_postulacion
    LEFT JOIN per_postulacion_conocimiento ppc ON pp.id_postulacion = ppc.postulante_id
    WHERE sp.id_persona = $persona_id")->fetch();

    $dependiente = $db->query("SELECT*
    FROM sys_persona sp 
    INNER JOIN per_postulacion pp ON sp.postulante_id = pp.id_postulacion
    LEFT JOIN per_postulacion_dependiente ppd ON pp.id_postulacion = ppd.postulante_id
    WHERE sp.id_persona = $persona_id")->fetch();

    $formacion = $db->query("SELECT*
    FROM sys_persona sp 
    INNER JOIN per_postulacion pp ON sp.postulante_id = pp.id_postulacion
    LEFT JOIN per_postulacion_formacion ppf ON pp.id_postulacion = ppf.postulante_id
    WHERE sp.id_persona = $persona_id")->fetch();
}
        ...
        <div class="card">
            <div class="card-body">
                <div class="user-avatar text-center d-block">
                    <img src="<?= ($_user['avatar'] == '') ? imgs . '/avatar.jpg' : files . '/profiles/usuario/' . $_user['avatar']; ?>" alt="User Avatar" class="rounded-circle user-avatar-xxl">
                </div>
                <div class="text-center">
                    <h2 class="font-24 mb-0"><?= escape($_user['nombres']); ?> <?= escape($_user['primer_apellido']); ?> <?= escape($_user['segundo_apellido']); ?></h2>
                    <p><?= escape($_user['rol']); ?></p>
                </div>
            </div>
            <div class="card-body border-top">
                <h3 class="font-16">Información de Usuario</h3>
                <div class="">
                    <ul class="list-unstyled mb-0">
                    <li class="mb-2"><i class="fas fa-fw fa-id-card mr-2"></i><?= escape($_user['username']); ?></li>
                    <li class="mb-2"><i class="fas fa-fw fa-envelope mr-2"></i><?= escape($_user['email']); ?></li>
    ...
    <div class="col-xl-9 col-lg-9 col-md-7 col-sm-12 col-12">
        ...
        <div class="influence-profile-content pills-regular">
            <ul class="nav nav-pills mb-3 nav-justified" id="pills-tab" role="tablist">
                <li class="nav-item">
                    <a class="nav-link active" id="pills-campaign-tab" data-toggle="pill" href="#pills-campaign" role="tab" aria-controls="pills-campaign" aria-selected="true">Información Personal</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" id="pills-packages-tab" data-toggle="pill" href="#pills-packages" role="tab" aria-controls="pills-packages" aria-selected="false">Información Profesional</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" id="pills-review-tab" data-toggle="pill" href="#pills-review" role="tab" aria-controls="pills-review" aria-selected="false">Información Adicional</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" id="pills-msg-tab" data-toggle="pill" href="#pills-msg" role="tab" aria-controls="pills-msg" aria-selected="false">Configuraciones</a>
                </li>
            </ul>
