...
$(function () {
	...
	var $dias = $('#dias'), $entrada = $('#entrada'), $salida = $('#salida');
    ...
    function listarHorario(){
        ...
        success: function(resp){
            ...
            var botones='';//'<a href="#" data-toggle="tooltip" data-title="Ver horario" class="btn btn-outline-info btn-xs" onclick="abrir_ver('+"'"+datos+"'"+');"><span class="fa fa-eye" ></span></a>';
...
//$("#select_roles").
    ...
    function abrir_actividad(datos){
        ...
        var activ=d[5];
         //alert('aqui'+activ);
    //preguntar si se eliminara?
         alertify.confirm('<span style="color:red">DESABILITAR HORARIO</span>', 'Deve estar con autorizacion de para esta accion, afecta al control de asistencia de la persona actual ¿Desea cambiar?', function(){
...
$(function () {
 ...
 var rutaproceso='?/s-feriados/procesos';
  
$("#formCrear").validate({
	rules: {fecha_inicio: {required: true},
        ...
        highlight: highlight,
        unhighlight: unhighlight,
		messages: {fecha_inicio: "Debe selecionar uan fecha.",
                   fecha_final: "Debe selecionar uan fecha.",
	...
	submitHandler: function(form){
		...
		$.ajax({
			type: 'POST',
			url: rutaproceso,//"?/rh-horarios/guardar",
			data: datos+'&accion='+'guardar_datos',
			success: function (resp) {
                //alert(resp);
				//console.log(resp);
				switch(resp){
					case '1': 
                           listartabla(); //dataTable.ajax.reload();
							...
							alertify.success('Se REGISTRO el feriado correctamente');
							break;
					case '2': //dataTable.ajax.reload();
							listartabla();$("#modal_crear").modal("hide"); 
							alertify.success('Se EDITO el feriado correctamente'); 
							break;
                    case '11': //dataTable.ajax.reload();
							//$("#modal_familiar").modal("hide"); 
							alertify.warning('deve SELECCIONAR DIAS para guardar'); 
