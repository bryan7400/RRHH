...
$id_punto = (isset($_params[1])) ? $_params[1] : 0;

//punto
$puntoActual = $db->query("SELECT * from gon_puntos p  where p.id_punto = $id_punto")->fetch_first();
//var_dump($puntoActual);exit();
// Obtiene el rutas
    ...
    <div class="row">
        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
            <div class="page-header">
                <h2 class="pageheader-title">Editar paradas</h2>
            ...
            <div class="card-body">
                <form method="post" action="?/puntos/guardar" autocomplete="off" id="puntoform">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text"   name="nombre_lugar" id="nombre_lugar" class="form-control" data-validation="required letternumber length" data-validation-allowing="-/.#() " data-validation-length="max250" value="<?=$puntoActual['nombre_lugar'];?>">
                    </div>
                    <div class="form-group">
                        <label for="descripcion" class="control-label">Descripcion:</label>
                        <input type="text" name="descripcion" id="descripcion" class="form-control" autofocus="autofocus"  value="<?=$puntoActual['descripcion'];?>">
                        
                        <input type="hidden" value="<?= $id_ruta; ?>" id="id_ruta" name="id_ruta"/>
                        <input type="hidden" value="<?=$puntoActual['id_punto'];?>" id="id_punto" name="id_punto"/>
                    </div>
                  <!--  <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <select name="estado" id="estado" data-validation="required number" class="form-control">
                           <?php 
                            ...
                            <option value="0"  <?=$inactive;?>  >INACTIVO</option>
                        </select>
                    </div>
                    <div class="form-group">
            ...
            <div class="card-header">
                <div class="card-header">
                    <div class="row">
                        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                            <div class="" role="alert">
                                <b>El punto negro en el mapa es el punto actual,</b><br>
                                <b>Selecione un punto nuevo en el mapa, y arrastrelo para corregirlo</b>
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
	var puntoActualLtd='<?=$puntoActual['latitud'];?>';
	var puntoActualLng='<?=$puntoActual['longitud'];?>';
...
// 		bgPos: (Point)
 ...
 /*   var lime1Icon = new LeafIcon({iconUrl: '<?//= files .'/puntero/lime1.png' ?>'}),
    ...
    var centerPoint = [puntoActualLtd, puntoActualLng];
...
//            L.polygon($rutas1).addTo(map).bindPopup(ruta[0]);
            ...
            function() {
                if(validaForm()){
                    ...
                    $.ajax({ //datos que se envian a traves de ajax
                        ...
                        dataType: 'json',
                        url:   '?/gon-rutas/guardar-punto', //archivo que recibe la peticion
                        data:  formData,
                        contentType: false,
                        processData: false
                    }).done(function (ruta) {
                        ...
                        if (ruta.estado == 's') {
                            ...
                            alertify.success('El punto fue ediado satisfactoriamente.');
	...
	var marcador = L.icon({
        //iconUrl:'assets/imgs/md_5b29c14a90f9e.jpg',
       iconUrl:'assets/imgs/pin.png',
		...
		var marker = L.marker([puntoActualLtd, puntoActualLng], {icon: marcador}).addTo(map);
		
	/* var markerscant = L.marker(([puntoActualLtd, puntoActualLng]),  { 
						icon: L.icon.glyph({ glyph:'Punto actual'}) 
					}).addTo(map);*/
...
/*	 var markers = L.Routing.control({
	...
	for (var i=1; puntos.length >i; i++) {
		...
		//marcamos cantidades en punto
 		if(parte[0]!=puntoActualLtd && parte[1]!=puntoActualLng){
