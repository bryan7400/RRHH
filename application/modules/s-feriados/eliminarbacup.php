...
if (is_post()) {
	...
	if (isset($_POST[get_csrf()])) {
		...
		$id_feriado = (isset($_params[0])) ? $_params[0] : 0;
		 ...
		 $id_dias_feriados = $_POST['id_dias_feriados']; 
		// Obtiene el sferiados
		$sferiados = $db->from('asi_dias_feriados')->where('id_feriado', $id_dias_feriados)->fetch_first();
		
		// Verifica si existe el sferiados
		if ($sferiados) {
			// Elimina el sferiados
			...
			if ($db->affected_rows) {
				...
				$db->insert('sys_procesos', array(
					...
					'proceso' => 'd',
					'nivel' => 'm',
					'detalle' => 'Se eliminó el sferiados con identificador número ' . $id_dias_feriados . '.',
					'direccion' => $_location,
