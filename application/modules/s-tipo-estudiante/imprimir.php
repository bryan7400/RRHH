...
if ($id_tipo_estudiante == 0) {
	...
	$pdf->Cell(0, 15, 'TIPO ESTUDIANTE', 0, true, 'C', false, '', 0, false, 'T', 'M');
	...
	foreach ($stipoestudiante as $nro => $stipoestudiante) {
		...
		$body .= '<td>' . ($nro + 1) . '</td>';
		$body .= '<td>' . escape($stipoestudiante['nombre_tipo_estudiante']) . '</td>';
		$body .= '<td>' . escape($stipoestudiante['descripcion']) . '</td>';
		$body .= '<td>' . escape($stipoestudiante['fecha_registro']) . '</td>';
		$body .= '<td>' . escape($stipoestudiante['gestion_id']) . '</td>';
		$body .= '</tr>';
	}
	...
	$tabla .= '<th width="6%">#</th>';
	$tabla .= '<th width="38.30%">Nombre tipo estudiante</th>';
	$tabla .= '<th width="19.15%">Descripcion</th>';
	$tabla .= '<th width="24.37%">Fecha registro</th>';
	$tabla .= '<th width="12.19%">Gestion</th>';
	$tabla .= '</tr>';
	$tabla .= $body;
	$tabla .= '</table>';
