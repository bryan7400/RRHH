...
$permiso_ver = in_array('imprimir', $_views);
...
//Obtiene compras
$compras = $db->query("SELECT * FROM inv_ingresos i WHERE i.fecha_ingreso = '$fecha' AND i.tipo='Compra' AND i.empleado_id = '" . $_user['empleado_id'] . "' group by id_ingreso")->fetch();
//cronograma de pagos
...
//pagos compras
$pagos_compras = $db->query("select p.id_pago, p.movimiento_id, i.fecha_ingreso, i.fecha_ingreso, i.tipo, i.id_ingreso, i.nombre_proveedor, i.monto_total, pd.metodo, ifnull(abono,0) as subtotal from inv_pagos p left join inv_pago_detalles pd on p.id_pago= pd.pago_id LEFT JOIN inv_ingresos i ON i.id_ingreso=p.movimiento_id where p.tipo='Ingreso' and pd.estado='1' and pd.empleado_id=" . $_user['empleado_id'] . " and pd.fecha_real='$fecha' and i.fecha_ingreso!=pd.fecha_real")->fetch();
...
list($ancho_header, $alto_header) = getimagesize(imgs . '/empty2.jpg');
...
class MYPDF extends TCPDF {
	public function Header() {
		...
		$imagenn = (IMAGEN_SUCURSAL != '') ? institucion . '/' . IMAGEN_SUCURSAL : imgs . '/empty2.jpg' ;
...
if ($cobros || $ingresos) {
	if ($cobros) {		
		foreach ($cobros as $nro => $cobro) {
			...
			$body .= '<td class="left-right">' . date_decode($cobro['fecha_real'], $_institution['formato']) . '</td>';
			...
			$body .= '<td class="left-right" align="right">' . $cobro['metodo'] . '</td>';
			$body .= '</tr>';
		}
	}
//ingresos
...
if ($ventas) {	
	foreach ($ventas as $nro => $venta) {
		...
		if ($venta['plan_de_pagos'] == 'si'){ 
			...
			$pagos = $db->query("select id_pago, movimiento_id, metodo, ifnull(SUM(monto),0) as subtotal from inv_pagos p left join inv_pago_detalles pd on p.id_pago= pd.pago_id where p.movimiento_id = '". $venta['id_egreso'] . "' AND pd.estado='1' AND p.tipo='Egreso' GROUP by movimiento_id")->fetch_first();
			if ($pagos) {
				...
				$body_ventas .= '<td class="left-right" align="right">' . $pagos['metodo'] . '</td>';					
			}else{
...
if ($egresos || $pagos_compras || $cronogramas) {
	if ($cronogramas) {		
		foreach ($cronogramas as $nro => $cronograma) {
			...
			$body_egresos .= '<td class="left-right">' . date_decode($cronograma['fecha_real'], $_institution['formato']) . '</td>';
			...
			$body_egresos .= '<td class="left-right" align="right">' . number_format($cronograma['monto'], 2, '.', '') . '</td>';
			$body_egresos .= '<td class="left-right" align="right">' . $cronograma['metodo'] . '</td>';
			$body_egresos .= '</tr>';
		}
	}
	...
	if ($pagos_compras) {		
		foreach ($pagos_compras as $nro => $pagos_compra) {
			...
			$body_egresos .= '<td class="left-right" align="right">' . $pagos_compra['metodo'] . '</td>';
			$body_egresos .= '</tr>';
		}
	}
//egresos
...
if ($compras) {	
	foreach ($compras as $nro => $compra) {
		...
		if ($compra['plan_de_pagos'] == 'si'){ 
			...
			$pagos = $db->query("select id_pago, movimiento_id, pd.metodo, ifnull(SUM(abono),0) as subtotal from inv_pagos p left join inv_pago_detalles pd on p.id_pago= pd.pago_id where p.movimiento_id = '". $compra['id_ingreso'] . "' AND p.tipo='Ingreso' AND pd.estado='1' GROUP by movimiento_id")->fetch_first();
			if ($pagos) {
				if ($pagos['metodo'] == 'Efectivo') 
					...
					$body_compras .= '<td class="left-right" align="right">' . $pagos['metodo'] . '</td>';					
				}else{
				...
				if ($compra['metodo'] == 'Efectivo') {
					...
					$body_compras .= '<td class="left-right" align="right">' . number_format($compra['monto_total'], 2, '.', '') . '</td>';
					$body_compras .= '<td class="left-right" align="right">' . number_format($compra['monto_total'], 2, '.', '') . '</td>';
					$body_compras .= '<td class="left-right" align="right">' . $compra['metodo'] . '</td>';											
				} else {
