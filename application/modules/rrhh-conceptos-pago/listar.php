...
$permiso_descuento = in_array('descuento', $_views);
						...
						<div class="btn-group">
								...
								<div class="input-group-append be-addon">
									...
									<div class="dropdown-menu">
										...
										<a href="#" onclick="abrir_pago();" class="dropdown-item">Concepto de Pago</a>
										<?php endif ?>  
										<?php if ($permiso_crear) : ?>
										<div class="dropdown-divider"></div>
										<a href="#" onclick="abrir_descuento();" class="dropdown-item">Concepto de Descuento</a>
										<?php endif ?>  
				...
				<table id="table" class="table table-bordered table-condensed table-striped table-hover" style="width:100%">
					<thead>
						<tr class="active">
							...
							<th class="text-nowrap">Tipo de Concepto</th>
					...
					<tfoot>
						<tr class="active">
							...
							<th class="text-nowrap text-middle">Tipo de Concepto</th>
...
//}
//if($permiso_descuento){
	require_once ("descuento.php");
//}
//if($permiso_descuento){
	require_once ("descuento_editar.php");
//}
?>
...
<?php if ($permiso_modificar) : ?>
	function abrir_editar(contenido){
		...
		$("#mes").val(d[2]); 	//mes
		$("#porcentajex").val(d[3]);	//porcentaje
		$("#montox").val(d[4]);	//monto
		$("#descripcion").val(d[5]);	//descripcion
		$("#tipo_fijo_porcentaje").val(d[6]);	//fijo o porcentaje		
		$("#btn_nuevo").hide();
		$("#btn_editar").show();

		TipoBono();
	}
	function abrir_editar_descuento(contenido){
		$("#form_concepto_descuento")[0].reset();
		//validator.resetForm();
		$("#modal_concepto_descuento").modal("show");
		$("#titulo_concepto_descuento").text("Editar ");
		$('#table tbody').off();
		var d = contenido.split("*");
		
		$("#id_concepto_descuento").val(d[0]);
		$("#nombre_concepto_descuento").val(d[1]);
		$("#mes_descuento").val(d[2]); 	//mes
		$("#porcentaje_descuentox").val(d[3]);	//porcentaje
		$("#monto_descuentox").val(d[4]);	//monto
		$("#descripcion_descuento").val(d[5]);	//descripcion
		$("#tipo_fijo_porcentaje_descuento").val(d[6]);	//fijo o porcentaje		
		
		$("#btn_nuevo_descuento").hide();
		$("#btn_editar_descuento").show();
...
<?php if ($permiso_descuento) : ?>
function abrir_descuento(){
	//console.log('jhola');
	$("#modal_concepto_descuento").modal("show");
	$("#form_concepto_descuento")[0].reset();
	//$("#form_gestion").reset();
	$("#titulo_concepto_descuento").text("Crear ");
	$("#btn_editar_descuento").hide();
	$("#btn_nuevo_descuento").show();
}
<?php endif ?>
	...
	"columnDefs": [
		{
			"render": function (data, type, row) {
				var result = "";
				var $tipo=row['tipo'];
				var contenido = row['id_concepto_pago'] + "*" + row['nombre_concepto_pago']+ "*" + row['mes']+ "*" + row['porcentaje']+ "*" + row['monto']+ "*" + row['descripcion']+ "*" + row['fijo_porcentaje'];
				
				<?php if ($permiso_ver){ ?>
					result+="<a href='#' class='btn btn-info btn-xs' onclick = 'ver("+'"'+contenido+'"'+");'><span class='icon-eye'></span></a>&nbsp";
				<?php } ?>
				
				<?php if ($permiso_modificar){ ?>
					if(row['tipo']=="BONO"){
						result+="<a href='#' class='btn btn-warning btn-xs' style='color:white' onclick='abrir_editar("+'"'+contenido+'"'+")'><span class='icon-note'></span></a>&nbsp";
					}
					else{
						result+="<a href='#' class='btn btn-warning btn-xs' style='color:white' onclick='abrir_editar_descuento("+'"'+contenido+'"'+")'><span class='icon-note'></span></a>&nbsp";
					}
				<?php } ?>
				
				<?php if ($permiso_eliminar){ ?>
					result+="<a href='#' class='btn btn-danger btn-xs' onclick='abrir_eliminar("+'"'+contenido+'"'+")'><span class='icon-trash'></span></a>";
				<?php } ?>
