...
$_ver_permiso = $db->query("SELECT * FROM ins_permisos insp WHERE estudiante_id='$estudiante_id'
	AND   date(insp.fecha_inicio) <= '$fecha_inicio'   AND date(insp.fecha_final)  <= '$fecha_final'")->fetch();
		...
		if ($nombre_archivo_documento != ''){
			$formatos_permitidos =  array('pdf', 'jpg', 'jpeg', 'png', 'docx');
 			...
 			if (!in_array($extension, $formatos_permitidos)) {
 				...
 				$output_dir = 'files/' . $nombre_dominio . '/permisos/';
 			...
 			if ($archivo_documento_nombre != ''){
 				unlink('files/' . $nombre_dominio . '/permisos/'.$archivo_documento_nombre);
 			} else {
