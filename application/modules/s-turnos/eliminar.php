...
if (is_post()) {
	...
	//if (isset($_POST[get_csrf()])) {
		...
		// Obtiene el modo
		$modo = $db->from('ins_turno')->where('id_turno', $id_turno)->fetch_first();
		
		$aula_paralelo="SELECT IFNULL(COUNT(*),0) contador FROM ins_aula_paralelo WHERE turno_id = $id_turno";
		...
		if ($modo && $sql_ins['contador'] == 0) {
			...
			$db->query("UPDATE ins_turno SET estado = 'I', usuario_modificacion = '".$_user['id_user']."', fecha_modificacion = '".$fecha_actual."' WHERE id_turno= '".$id_turno."'")->execute();
			...
			if ($db->affected_rows) {
				...
				$db->insert('sys_procesos', array(
					...
					'proceso' => 'd',
					'nivel' => 'm',
					'detalle' => 'Se eliminó la el turno con identificador número ' . $id_turno . '.',
					'direccion' => $_location,
