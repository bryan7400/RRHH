...
if (is_post()) {
	...
	//if (isset($_POST[get_csrf()])) {
		...
		// Obtiene el modo
		$modo = $db->from('ins_tipo_documentos')->where('id_tipo_documento', $id_materia)->fetch_first();
		...
		if ($modo) {
			...
			$db->query("UPDATE ins_tipo_documentos SET estado = 'I', usuario_modificacion = '".$_user['id_user']."', fecha_modificacion = '".$fecha_actual."' WHERE id_tipo_documento = '".$id_materia."'")->execute();
			...
			if ($db->affected_rows) {
				...
				$db->insert('sys_procesos', array(
					...
					'proceso' => 'd',
					'nivel' => 'm',
					'detalle' => 'Se eliminó la el documento con identificador número ' . $id_materia . '.',
					'direccion' => $_location,
