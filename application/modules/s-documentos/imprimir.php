...
if ($id_materia == 0) {
	// Obtiene los materia
	$materia = $db->select('z.*')->from('ins_tipo_documentos z')->order_by('z.nombre', 'asc')->fetch();

	// Ejecuta un error 404 si no existe los materia
	if (!$permiso_listar) { require_once not_found(); exit; }
} else {
	// Obtiene el materia
	$materia = $db->select('z.*')->from('ins_tipo_documentos z')->where('z.nombre', $id_materia)->fetch_first();
...
if ($id_materia == 0) {
	...
	$pdf->Cell(0, 15, 'DOCUMENTO', 0, true, 'C', false, '', 0, false, 'T', 'M');
	...
	foreach ($materia as $nro => $materia) {
		...
		$body .= '<td>' . ($nro + 1) . '</td>';
		$body .= '<td>' . escape($materia['nombre']) . '</td>';
	...
	$tabla .= '<th width="6%">#</th>';
	$tabla .= '<th width="13.29%">Nombre documento</th>';
