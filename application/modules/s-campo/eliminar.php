...
if (is_post()) {
	...
	//if (isset($_POST[get_csrf()])) {
		...
		$id_campo = (isset($_POST['id_campo'])) ? $_POST['id_campo'] : 0;
		// Obtiene el modo
		$modo = $db->from('pro_campo')->where('id_campo', $id_campo)->fetch_first();
		
		$campo_con="SELECT IFNULL(COUNT(*),0) contador FROM pro_materia WHERE campo_id = $id_campo";
		$sql_ins = $db->query($campo_con)->fetch_first();
		
		// Verifica si existe el modo
		if ($sql_ins['contador'] == 0) {
			$fecha_actual = date('Y-m-d H:i:s');//obtiene la fecha actual

			//ejecuta la eliminacion logica de la modo de calioficación 
			$db->query("UPDATE pro_campo SET estado = 'I', usuario_modificacion = '".$_user['id_user']."', fecha_modificacion = '".$fecha_actual."' WHERE id_campo = $id_campo")->execute();
			...
			if ($db->affected_rows) {
				...
				$db->insert('sys_procesos', array(
					...
					'proceso' => 'd',
					'nivel' => 'm',
					'detalle' => 'Se eliminó el campo con identificador número ' . $id_campo . '.',
					'direccion' => $_location,
